\chapter{Topology meets Analysis}\label{chap:baire}

Here we discuss two purely topological notions that have applications in mathematical analysis.
These results will not be used further in the text.

\section{Baire theorem}

A topological space $\c{X}$ is called \index{Baire space}\emph{Baire} if any countable family of dense open sets in $\c{X}$ has a dense intersection.
...
The following theorem implies that the real line $\RR$ is Baire.
...
Since $V_1$ is dense and open, we can choose another ball $B_1=\Ball(p_1,r_1)\subset B_0\cap V_1$;
moreover, taking the radius $r_1$ smaller we can achieve that $B_1\Subset B_0\cap V_1$ and $r_1\le\tfrac12\cdot r_0$.
Let us continue this process for $V_2$, $V_3,\dots$;
...
Theorem \ref{thm:baire-category} is the main source of Baire spaces;
the following exercise provides another source.
...
Show that the irrational numbers $\RR\setminus\QQ$ form a Baire space.
\end{thm}

\begin{thm}{Exercise}\label{ex:union-closures}
Let $B_1,B_2,\ldots$ be a sequence of nonempty sets in a Baire space $\c{X}$.
Assume $\c{X}=B_1\z\cup B_2\cup\ldots$
...
Any open set in a Baire space is Baire.
\end{subthm}
...
We say that $G$ is a \index{G-delta set}\emph{G-delta} set if it can be written as a countable intersection of open sets;
...
\[V_1,\ V_1\cap V_2,\ V_1\cap V_2\cap V_3,\ \ldots\]
...
\begin{thm}{Observation}\label{obs:uG-delta}
The union of two (and therefore finitely many) G-delta sets is G-delta.
\end{thm}

\parit{Proof.}
The observation follows from the identity
...
Indeed, we may assume that $V_i$ and $W_j$ are open and our G-delta sets are $G_1=\bigcap_i V_i$, and $G_2=\bigcap_j W_j$, so the identity gives two expressions for $G_1\cup G_2$.
Since the set $V_i\cup W_j$ is open for any $i$ and $j$, the right-hand side says that $G_1\cup G_2$ is G-delta.
\qeds


Recall that \textit{countable union of countable sets is countable}.
Since each G-delta set is a countable intersection of open sets,
we get the following.

\begin{thm}{Observation}\label{obs:nG-delta}
A countable intersection of G-delta sets is G-delta.
\end{thm}
...
Then a countable intersection of dense G-delta sets in $\c{X}$ is dense.
\end{thm}
...
Recall that $\QQ\subset\RR$ denotes the set of rational numbers.
...
On the other hand, $\QQ\subset\RR$ is a countable union of one-point sets; each one-point set $\{q\}$ can be written as an intersection of open intervals $(q-\tfrac1n,q+\tfrac1n)$.
Therefore, a countable union of G-delta sets does not have to be G-delta.
...
Let us enumerate all rational numbers $r_1,r_2,\ldots$
Consider the function $f\: \RR\to \RR$ defined by $f(r_n)=\tfrac1n$ and $f(x)=0$ for any irrational $x$.
...
Observation \ref{obs:nG-delta} can be interpreted the following way:
\textit{dense G-delta sets contain all points in a Baire space except a negligible set}.
At least, we cannot cover an open set with complements of dense G-delta sets
and the intersection of countably many dense G-delta sets is a dense G-delta set.

In spaces with measure, the sets of full measure have analogous features, but as we will see in an incompatible way.

Let us enumerate rational numbers $q_1,q_2,\ldots$; consider the unions of open intervals:
\[V_n\df \bigcup_i (q_i-\tfrac1{2^{n+i}},q_i+\tfrac1{2^{n+i}}).\]
Note that $G=V_1\cap V_2\cap \ldots$ is a dense G-delta set in $\RR$.
Observe that the \index{Lebesgue measure}\emph{Lebesgue measure}%
\footnote{Lebesgue measure of a set in $\RR$ can be thaut as its total length, but it is defined on a much larger class of subsets that include all G-delta sets and Borel sets that will be discussed soon.}
of $V_n$ is less than
\[\frac1{2^{n-1}}=\frac{2}{2^{n+1}}+\frac{2}{2^{n+2}}+\ldots\]
Therefore, $G$ has vanishing measure.
So, measure-theoretically, $G$ is nothing, but topologically it is everything.
...
Since the complement of a countable union is the countable intersection of the complements,
a sigma-algebra can be equivalently defined using countable intersection instead of countable union.
It follows that Borel sets include all closed sets (since they are complements of open sets) as well as G-delta sets (as countable intersections of open sets) and their complements, which are called \index{F-sigma set}\emph{F-sigma} sets;
the F-sigma sets can be defined directly as a union of countable families of closed sets.

The following proposition implies that any Borel set in $\RR$ is a negligible modification of an open set.
...
that is, a subset $S\subset \c{X}$ belongs to $\s{S}$ if there is a dense G-delta set $G\subset\c{X} $ and an open set $V\subset\c{X}$ such that $S\cap G=V\cap G$.
...
Furthermore, $G$ is G-delta, and since $\c{X}$ is Baire, it must be dense.
Observe that
\[S\cap G=V\cap G;\]
...
To construct a Vitali set, consider the equivalence relation $\sim$ on $\RR$ such that $x\sim y$ $\Leftrightarrow$ $x-y\in \QQ$
and choose one number from each equivalence class.
Be aware that to \textit{choose} a number in every equivalence class one needs the axiom of choice.
...
\[\bigcap_{q\in \QQ}(q+G)\subset \RR\setminus \bigcup_{q\in \QQ}(q+L)=\emptyset,\]
...
therefore, we can choose small positive $q\in\QQ$ such that $V_q=V\cap (q+V)$ is a nonempty open set.
...
Choose $x\in [0,1]$.
Observe that $f_1(x), f_2(x),\ldots$ is a Cauchy sequence of reals; so,
...
to prove the latter statement, take huge value $m$ and approximate a given function by a zig-zag-looking function with large slope as shown in the picture.%
\footnote{More formally, let us choose a small $\eps>0$.
Given a continuous function $f$ we can choose large $m$ so that if $|x_0-x_1|\le \tfrac1m$, then $|f(x_0)-f(x_1)|<\eps$.
Now define a new function $h$ by setting $h(\tfrac im)=f(\tfrac im)+(-1)^i\cdot\eps$ for an integer $i$ from $0$ to $m$ and extend $h$ linearly on each interval $[\tfrac {i-1}m,\tfrac im]$.
...
is undefined.
Therefore $f$ differentiable at no point $x_0\in [0,1]$.
...
However, finding one such example is challenging.
This is very common in mathematics: as soon as you define a typical object
in your favorite theory, you realize that you have never actually seen one.
(By the way, did you ever see a typical man?)
...
\item A typical compact set in the plane is homeomorphic to the Cantor set; see \ref{sec:Cantor set}.
Furthermore, a typical compact \textit{connected} set (defined in the next chapter) is homeomorphic to the pseudo-arc, which is a truly weird example in many ways (read about~it).
\end{itemize}
