...
%??? ADD\section{Borel sets}

%Any topological set comes with the so-called \index{Borel sigma-algebra}\emph{Borel sigma-algebra} that is the minimal family of sets that include all open sets and closed under complement, countable union.
%Sets in Borel sigma-algebra are called \index{Borel set}\emph{Borel}.
%Evidently Borel sets are closed under countable intersections.

%Note that Borel sets include all closed sets and G-delta set.
%The same holds for the complements of G-delta; they are called \index{F-sigma set}\emph{F-sigma} sets and can be defined directly as intersections of countable families of closed sets.

%Borel sets play a key role in analysis.
%Since it is a purely topological notion it worths to mention them, but we will not going discuss them further in the text.
%The following two exercises might help you to get more familiar with the definition.

%\begin{thm}{Exercise}
%Construct a continuous map $f\:\c{X}\to \c{Y}$ and a Borel set $B\subset \c{X}$ such that $f(B)$ is not Borel.
%\end{thm}

%\begin{thm}{Advanced exercise}
%Construct a bijection $f\:[0,1]\to[0,1]\times [0,1]$ such that $f$ and its inverse $f^{-1}$ map Borel sets to Borel sets.
%\end{thm}

%\begin{thm}{Advanced exercise}
%Show that for any Borel set $B$ in a Baire space there is an dense G-delta set $G$ and an open set $V$ such that $B\cap G=V\cap G$.
%\end{thm}
...
\item  A typical compact set in the plane is homeomorphic to the Cantor set. Furthermore, a typical compact \textit{connected} set (defined in the next chapter) is homeomorphic to the pseudo-arc, which is a truly weird example in many ways (read about it).
\end{itemize}
