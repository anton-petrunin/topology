...
For example, the space of rational numbers $\QQ$ is not a Baire.
...
Given two subsets $A$ and $B$ of a topological space, we will write \index{$A\Supset B$}$A\Supset B$ (equivalently, $B\Subset A$) if $\mathring{A}\supset \bar B$; that is, the interior of $A$ contains closure of $B$.
...
Show that for any open set $V\subset \c{K}$ there is an open set $W\Subset V$.
\end{subthm}
\begin{subthm}{ex:haus-comp-bair:bair}
Use part \ref{SHORT.ex:haus-comp-bair:Subset} and \ref{ex:nested-compact} to show that $\c{K}$ is a Baire space.
\end{subthm}
...
Let $\c{X}=\bigcup_{n=1}^\infty B_n$.
Show that if $\c{X}$ is a nonempty Baire space, then at least one of the sets $\bar B_n$
...
Observe that countable intersection of G-delta sets is G-delta.
The following identity implies that  union of two G-delta sets is G-delta:
...
\section{Large and small subsets}
...
The sets of full measure in spaces with measure have an analogous feature, but as we will see in an incompatible way.
...
Observe that the Lebesgue measure of $V_n$ (its total length) is less than $\tfrac1{2^{n-1}}$.
Therefore the G-delta set $G=V_1\cap V_2\cap \ldots$ has vanishing measure,
so it is nothing with respect to measure theory and everything with respect to topology.


\section{An application}

Let $\c{C}$ be the set of all continuous functions $[0,1]\to\RR$,
equipped with the \index{supnorm metric}\emph{supnorm metric}
...
Observe that $V_n$ is open and dense in $\c{C}$;
to prove the latter statement, take huge value $m$ and approximate a given function by zig-zag-looking function with large slope.
...
Likely, you did not see such function before, but you know many functions --- apparently they are very nontypical in this sense.
Here one more statement that can be proved via the same technology; see~\cite{kirchheim-spadaro-szekelyhidi}.

\begin{thm}{Claim}
In the space of distance nonexpanding maps $\SS^2\to\RR^2$, equipped with supnorm metric, the set of length-preserving maps forms a G-delta dense subset; a map $f\:\SS^2\to\RR^2$ is \index{length-preserving map}\emph{length-preserving} if $\length f\circ\gamma=\length \gamma$ for any curve $\gamma\:[a,b]\to \SS^2$.
\end{thm}

According to the claim there are length-preserving maps $\SS^2\to\RR^2$, but
finding just one such a map is a challenging problem.
It is a typical situation: it is hard to see the typical objects and these objects often have surprising properties.
