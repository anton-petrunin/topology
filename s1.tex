...
Conclude that any homeomorphism $\DD^2\to\DD^2$ maps the boundary $\partial D$ to itself.
\end{subthm}
...
A continuous map $\tilde f\:\c{X}\to \RR$ will be called \emph{lifting} of $f$ if $f=e\circ \tilde f$;
in other words if the following diagram \emph{commutes};
\begin{figure}[h!]
\centering
\begin{tikzcd}
& \RR \arrow[d, "e"]\\
...
The integer $\tilde f(1)$ will be called \emph{degree} of the loop $f$, it will be denoted by $\deg f$.
