...
Surjectivity of $\deg\:\pi_1(\SS^1,1)\cong\ZZ$ follows follows since given the path $f_n\:t\mapsto e(n\cdot t)$ has degree $n$.
...
Recall that $e(t)=\exp(2\cdot\pi\cdot i\cdot t)$.
Given $r\ge 0$, consider the loop
\[f_r\:t\mapsto p(r\cdot e(t))/|p(r\cdot e(t))|\]
in $\SS^1$.
Observe that $f_0$ is a constant loop.
Note that $f_r$ converges to $e(k\cdot t)$ as $r\to\infty$, here $k$ is
the degree of $p$.
In particular, if $r$ is large then $f_r(t)-e(k\cdot t)\ne 0$ for any $t$;
arguing as in \ref{ex:homot-sphere}, we get that $f_r$ is homotopic to path $t\mapsto e(k\cdot t)$ for large $r$.
