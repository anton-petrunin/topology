...
Let $u\:\tilde{\c{Z}}\to \c{Z}$ be a covering with connected total space $\tilde{\c{Z}}$.
This covering is called \emph{universal} if any other covering $w\:\c{Y}\to \c{Z}$ with connected total space can be included in the commutative diagram of coverings.

Soon we will see that every reasonable connected space $\c{Z}$ admit a universal covering;
moreover, a covering $u\:\tilde{\c{Z}}\to \c{Z}$ is universal if and only if the total space $\tilde{\c{Z}}$ is simply-connected.

The term reasonable will be used only in this lecture; it is not standard by all means.
We will use it as a shortcut for \index{locally simply-connected}\emph{locally simply-connected}, but there are other options.
So, $\c{Z}$ is \index{reasonable space}\emph{reasonable} if for any open set $V\subset \c{Z}$ and any $p\in V$ there is an open simply-connected set $W$ such that $V\supset W\ni p$.
...
By \ref{ex:retract*:r},  $V$ is not simply-connected.
...
\emph{Polish circle} $\c{P}$ is a simply-conneced space that is not locally path-connected.
It is shown on the right side of the picture;
...
Since $\tilde{\c{Z}}$ is reasonable, it is also path-connected; see \ref{thm:open-connected=path-connected}.
By \ref{thm:covering-open}, $w$ is open.
By \ref{ex:covering-clopen}, $w(\tilde{\c{Z}})$ is a clopen subset in $\c{Z}$.
...
Assume contrary, then there are two points $\tilde z_0,\tilde z_1\in\tilde{\c{Z}}$ that map to a single point, say $z_0=w(\tilde z_0)=w(\tilde z_1)$.
...
Before reading the proof, refresh the the definition of fibered product from \ref{sec:Fibered product}.
...
Consider the fibered product of $f$ and $w$; see the diagram.
...
Assume $V\cap W$ has at least two connected components.
Glue a nontrivial 2-fold cover of $\c{Z}$ from two copies of $V$ and two copies of $W$.
Conclude that $\c{Z}$ has nontrivial fundamental group.
\end{thm}
...
We plan to construct a \textit{set} $\tilde{\c{Z}}$ with a map $u\:\tilde{\c{Z}}\to\c{Z}$, then equip $\tilde{\c{Z}}$ with a topology and check that $u$ is indeed a universal covering.
...
Since $\c{Z}$ is locally simply-connected, this implies that $u$ is a covering.
...
Choose a point $\tilde z=[a]\in \tilde{\c{Z}}$ for some $a\in\Path(\c{Z},z_0)$.
...
Sine $\tilde a$ is a loop, $a=a_1$ and $e_{z_0}=a_0$ we have that
...
Let $\c{G}$ be a \index{topological group}\emph{topological group};
assume it is reasonable and connected and $u\:\tilde{\c{G}}\to \c{G}$ is its universal covering.
...
Suppose $\c{Z}$ is a reasonable and $\tilde{\c{Z}}$ is simply-connected.
...
Given $\alpha\in \pi_1(\c{Z},z_0)$,
choose a loop $a\:[0,1]\to \c{Z}$ such that $\alpha=[a]$,
lift it to a path $\tilde a\:[0,1]\to \tilde{\c{Z}}$ and set
...
Therefore $\c{Z}$ is precisely the quotient of $\tilde{\c{Z}}$ by this action, equipped with the quotient topology, which proves the statement.
\qeds
...
There is also a combinatoric definition; see for example ???.
...
Proving equivalence of the definitions of free groups takes some effort, and the proof below should include them,
...
monodromy homomorphism --- homomorphism to the quotient group
