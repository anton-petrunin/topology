...
The proof is based on the following partial case of the Van Kampen theorem.
...
Conclude that the sphere $\mathbb{S}^2$ is simply-connected.
\end{thm}
...
The following exercise provides a partial case of the so-called \index{Tietze--Urysohn extension theorem}\emph{Tietze--Urysohn extension theorem}.

\begin{thm}{Exercise}\label{ex:tietze}
Let $\phi$ be a continuous map from a closed subset $\Gamma\subset\RR^2$ to the open interval $(1,2)$.
Given $x\in\RR^2$, set
\[
a(x)=\inf\limits_{y\in\Gamma} \{\,|x-y|\,\}
\quad\text{and}\quad
b(x)=\inf\limits_{y\in\Gamma} \{\,\phi(y)\cdot |x-y|\,\}.
\]
Show that
\[f(x)=
\begin{cases}
\phi(x)& \text{if}\ x\in \Gamma,
\\
\frac{b(x)}{a(x)}& \text{if}\ x\notin \Gamma
\end{cases}
\]
defines a continuous function $f\:\RR^2\to (1,2)$.

Conclude that if there is a homeomorphism $h\:\RR\to\Gamma$,
...
By \ref{ex:tietze}, there is a function $f\:\RR^2\z\to\RR$ such that
...
Indeed, $G$ is continuous and it has an inverse
...
This theorem is a partial case of the famous Jordan theorem;
it is known for its simple formulation and annoyingly tricky proofs.
The presented proof is due to Patrick Doyle \cite{doyle};
