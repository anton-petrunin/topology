...
A collection $\{V_\alpha\}$ of open subsets of a topological space $\c{X}$ is called its \index{open cover}\emph{open cover} if it covers the whole $\c{X}$;
...
A subcollection of the cover $\{V_\alpha\}$ that is also a cover is called its \index{subcover}\emph{subcover} of $\{V_\alpha\}$.
\end{thm}
...
Let us show how to apply the definition.
Suppose $S\subset \RR$ be a compact set;
let us show that $S$ is closed.
Assume the contrary,
then we can choose a point $s\in\bar S\setminus S$ and consider the cover of $S$ by intervals $(-\infty, s-\eps)$ and $(s+\eps,+\infty)$ for all $\eps>0$.
...
These two statements will be generalized soon; the following two exercises have similar solutions.
...
Show that $f$ is bounded; that is there is a constant $c$ such that $|f(x)|\le c$ for any $x\in\c{K}$.
\end{subthm}
...
\footnote{Such examples cannot be metrizable.}
\end{subthm}
...
A set $V$ in $\c{X}^\infty=\c{X}\cup\{\infty\}$ is declared to be open if $V$ is an open subset in $\c{X}$ (in this case $\infty\notin V$) or $\infty\in V$ and its complement $\c{X}\setminus V$ is a closed \textit{and} compact subset of $\c{X}$.

We need to check the conditions in \ref{def:top-space}:
\ref{SHORT.def:top-space:empty} and \ref{SHORT.def:top-space:u} are straightforward and \ref{SHORT.def:top-space:n} follows since union of two closed sets is closed and union of two compact sets is compact; see \ref{ex:ucompact}.
\qeds
...
A topological space $\c{X}$ is compact if and only if for any collection of closed sets $\{Q_\alpha\}$ in $\c{X}$ such that
...
According to the proposition, compactness is equivalent to the following property: \textit{any collection of closed sets with finite intersection property has a nonempty intersection}.
...
Add to it $W$; that is, consider the collection of sets that includes $W$ as well as $V_\alpha$ for all $\alpha\in\c{I}$.
...
Finally observe that products $V_{\alpha_i}\times V_{\alpha_j'}$ cover $\c{X}\times\c{Y}$.
\qeds
...
otherwise the union of $V_\alpha\times  \c{Y}$ and $\c{X}\times W_\beta$ would not cover $\c{X}\times \c{Y}$.
Therefore, we can assume that $\{V_\alpha\}$ cover $\c{X}$.
Since $\c{X}$ is compact we can choose a finte subcover $\{V_{\alpha_1},\dots,V_{\alpha_n}\}$.
...
Let us show one more application of this theorem.
...
Equip $\Power(S)$ with topology defined by prebase $\{\mathfrak{V}_s,\mathfrak{W}_s\}_{s\in S}$, now we can call it \index{power space}\emph{power space}.
Applying the argument above one can see that $\Power(S)$ is a compact space for \textit{any} set $S$.
This is a partial case of the \index{Tychonoff compactness theorem}\emph{Tychonoff compactness theorem}.

For finite set $F$ the space $\Power(F)$ is a descrete space with $2^{|F|}$ points.
For the set of natural numnbers $\NN=\{1,2,\dots\}$ its power space $\Power(\NN)$ is homeomorphic to the Cantor set $\c{C}\subset [0,1]$.
(The set $\c{C}$ constructed as an intersection of nested sequence of subsets $[0,1]\z=\c{C}_0\supset \c{C}_1\supset \dots$ such that $\c{C}_{i+1}$ is obtained from $\c{C}_i$ by deleting the open middle third from every line segment in it.)
The following exercise describes a tryully scare example of compact space.

\begin{thm}{Advnced exercise}\label{ex:power-power}
Let $\c{A}=\Power(\NN)$ and $\c{B}=\Power(\c{A})$; as we know $\c{B}$ is a compact space.
Show that the following sequence of points $\mathfrak{p}_1$, $\mathfrak{p}_2,\dots$ in $\c{B}$ does not have a converginge subsequence:
