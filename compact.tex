...
A subset of cover $\{V_\alpha\}$ that is also a cover is called its \index{subcover}\emph{subcover} of $\{V_\alpha\}$.
\end{thm}
...
Applying the same reasoning for cover of $S$ by all sets intervals $(-c,c)$, one can show that $S$ be an bounded; that is, there is $c\in \RR$ such that $|s|\le c$ for any $s\in S$.
These two staements will be generalized soon; the following two exercises have similar solutions.
...
\begin{thm}{Claim}
Every topological space is homeomorphich to subspace of a compact space.
\end{thm}
...
\ref{SHORT.def:top-space:empty} and \ref{SHORT.def:top-space:u} are straightforward and \ref{SHORT.def:top-space:n} follows since union of two closed sets is closed and union of two compact sets is compact \ref{ex:ucompact}.
\qeds
...
A space $\c{X}$ is compact if for any collection of closed sets $\{Q_\alpha\}$ in $\c{X}$ such that
...
that is, $\{V_\alpha\}$ is an open cover of $\c{X}$.
...
Recall that the \mbox{\index{Cantor set}\emph{Cantor set}} is constructed the following way:
we start with the unit interval $[0,1]$, subdivide it into three equal inervals and remove the interior of the middle one.
Repeat this procedure recursively for each of the remaining closed intervals.
The following exercise and Theorem~\ref{thm:comp-interval} below will imply that the Cantor set is not empty.
...
\begin{thm}{Exercise}\label{ex:proper-point}
...
Thus $\{V_\alpha\}$ must have no finite subcovers of \textit{at least one} of these subintervals;
...
\begin{thm}{Exercise}\label{ex:alexander+product}
Use the Alexander prebase theorem to give a short proof of \ref{thm:compact-product}.
\end{thm}
