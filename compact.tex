...
The proof of the if part goes along the same lines as the proof that \textit{any bounded sequence of real numbers has a converging subsequence}, but it goes on a more abstract level.
The following exercise will guide you thru this proof.

\begin{thm}{Exercise}
Let $\c{M}$ be a compact metric space.

\begin{subthm}{}
Show that $\c{M}$ is bounded; that is, the distance between any pair of points in $\c{M}$ cannot exceed a some fixed real number, say $D$.
\end{subthm}

Let $x_n$ be a sequence of points in $\c{M}$.

\begin{subthm}{}
Suppose a closed ball $\cBall[p,R]$ contains an infinite set of elements of the sequence $x_n$.
Show that there is $p'\in \cBall[p,R]$ such that the closed ball $\cBall[p',R/2]$ contains an infinite set of elements of the sequence $x_n$.
\end{subthm}

\begin{subthm}{}
Conclude that there is a nested sequence of closed balls 
\[\cBall[p_0,1]\supset \cBall[p_1,\tfrac12]\supset\dots\supset \cBall[p_n,\tfrac1{2^n}]\supset\dots \]
such that each $\cBall[p_n,\tfrac1{2^n}]$ contains in an infinite set of elements of the sequence $x_n$.
\end{subthm}

\begin{subthm}{}
Apply the finite intersection property to $\cBall[p_n,\tfrac1{2^n}]$ to show that the sequence $p_n$ converges;
denote its limit by $p_\infty$.
\end{subthm}

\begin{subthm}{}
Show that a subsequence of $x_n$ convereges to $p_\infty$.
\end{subthm}
...
\noindent{\footnotesize Hint: Show and use that there is a finite cover of $\c{M}$ by balls $\Ball(p_i,r_i)$ such that for each $i$ the ball $\Ball(p_i,2\cdot r_i)$ lies in some $V_{\alpha_i}$.}
\end{thm}
