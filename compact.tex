...
In this chapter we define compact topological spaces --- especially nice spaces that behave in many ways like finite sets.


In practice, compactness is a powerful replacement for ``closed and bounded'' in $\RR^n$, but the general definition does not resemble it at first glance.
On compact spaces continuous functions behave well: they are bounded and attain their maxima and minima (\ref{ex:fun(compact)}).
Moreover, many limiting and convergence arguments become simpler and more robust.
...
\begin{thm}{Exercise}
Let $S\subset \RR$ be a compact subset.

\begin{subthm}{ex:closed-compact}
Show that $S$ is closed.
\end{subthm}

\begin{subthm}{ex:unbounded-noncompact} Show that $S$ be an bounded;
 that is, for there is $c\in \RR$ such that $|s|\le c$ for any $s\in S$.
\end{subthm}
...
As we mentioned, comact spaces are close reatives of finite sets,
but, as you can see from the following statement, this relation is not too direct.
...
A set $V$ in $\hat{\c{X}}=\c{X}\cup\{\infty\}$ is declared to be open if $V$ is an open subset in $\c{X}$ (in this case $\infty\notin V$) or $\infty\in V$ and its complement $\c{X}\setminus V$ is a closed and compact subset of $\c{X}$.

We need to check the conditions in \ref{def:top-space}:
\ref{SHORT.def:top-space:empty} and \ref{SHORT.def:top-space:u} are straightforward and \ref{def:top-space:n} follows since union of two closed sets is closed and \ref{ex:ucompact}.
\qeds

The space $\hat{\c{X}}$ constructed in the proof is called the \index{one-point compactification}\emph{one-point compactification} of $\c{X}$.
...
\mbox{\index{Cantor set}\emph{Cantor set}} is constructed the following way:
start with the unit interval $[0,1]$, subdivide it into three equal inervals and remove the interior of the middle one.
Repeat this procedure recursively for each of the remaining closed intervals.
Observe that the following exercise imlies that the Cantor set is not empty.
...
\footnote{This type of reasoning is useful in all branches of mathematics;
sometimes it is called the \index{17 camels trick}\emph{17 camels trick}~\cite{17-camels}.
The name comes from the following mathematical parable:
...
We omit the proof of the following theorem, but it is worth knowing this result.
