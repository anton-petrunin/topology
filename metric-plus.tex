\chapter{Metric spaces revisited}

Recall that any metric space comes with a natural topology.
...
it will be done in \ref{sec:seq-comp-if} after proving asequence of auxiliary statements.
...
Since the sequence is infinite, this cover does not have a finite subcover, hence the only-if part follows.
\qeds
...
The Cauchy test (which is assumed to be known from calculus) implies that the real line $\RR$ with the standard metric is a complete space.
...
Topological spaces that admit a countable base are called \index{second-countable space}\emph{second-countable}.\footnote{A space is called \index{first-countable space}\emph{first-countable} if for each point $x$ there exists a sequence of neighbourhoods $U_1$, $U_2,\dots$ of $x$ such that for any open set $V\ni x$ we can chose $n$ such that $V\supset U_n\ni x$. For example, any metric space is first-countable; indeed, one can take $U_n=\Ball(x,\tfrac1n)$.}
...
By \ref{lem:lind}, we can pass to its countable countable, say $\{V_1,V_2,\dots\}$.
Assume it does not have a finite subcover.
