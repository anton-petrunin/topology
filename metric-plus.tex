...
The number $\eps$ in the lemma is called the \emph{Lebesgue number} of the cover;
this is a very useful characteristic of an open cover.
...
A topological space is called \emph{sequentially compact} if every sequence in it has a converging subsequence.
...
The if part requires deeper diving into metric spaces; it will be done in \ref{sec:seq-comp-if} after proving auxiliary statements in the following two sections.
...
Indeed, if this property holds, then we can choose $i_1$ such that $x_{i_1}\in \Ball(p,1)$, further $i_2>i_1$ such that $x_{i_2}\in \Ball(p,\tfrac12)$ and so on; on the $n$\textsuperscript{th} step we get $i_n>i_{n-1}$ such that $x_{i_n}\in \Ball(p,\tfrac1n)$.
...
For example, as it follows from the Cauchy test, the real line $\RR$ with the standard metric is a complete space.
On the other hand, an open interval $(0,1)$ forms a noncomplete subspace of $\RR$;
indeed, the sequence $x_n=\tfrac 1{2\cdot n}$ is Cauchy, it also converges to zero in $\RR$ which not a point of the subspace $(0,1)$.
...
Any of its subsequence has the same property; in particular none of its subsequences converges --- a contradiction.
\qeds
...
For gereral topological space sequential compactness does not imply compactness and the other way around, but we are not going to dicuss this relation.

Here are some standard examples:
The set of all countable ordinals, with the order topology and the so-called \emph{long ray} provide examples of topological space that is sequentially compact but not compact.
Examples of compact, but not sequentially compact spaces include $\{0,1\}^\RR$ --- the product space of continuum copies of the descrete two-point space and $\beta\NN$ --- Stone--Čech compactification of natural numbers.
In fact all the mentioned examples are Hausdorff (as defined in the next chapter), so limits of sequence behave in a reasonable way.
