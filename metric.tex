...
it is called \index{subspace}\emph{subspace} of $\c{X}$.
...
Show that $f$ is \index{injective map}\emph{injective}; that is, if $x\ne x'$ then $f(x)\ne f(x')$.
\end{subthm}
...
and $r>0$.
The set of points in $\c{X}$ which lies on the distance smaller than $r$ is called \index{ball}\emph{open ball} of radius $r$ centered at $x$.
...
The ball $\Ball(x,r)$ is also called \index{neighborhood}\emph{$r$-neighborhood} of $x$.
