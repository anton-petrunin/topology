...
The set $\mathbb{R}$ of all real numbers with the metric defined by $|x-y|$.
...
and 
\[h=g\circ f\:\c{X}\to \c{Z}\] is their \index{composition}\emph{composition};
that is, $h(x)=g(f(x))$ for any $x\in \c{X}$.
Show that $h\:\c{X}\to\c{Z}$ is continuous.
\end{thm}
...
Indeed a $1$-neighborhoo of any point $p$ is the opne-point set $\{p\}$, and it lies in any set containing $p$.
...
Equivalenltly, $\Ball(y,r)\z\subset\Ball(x,R)$.
Hence the result.
\qeds
