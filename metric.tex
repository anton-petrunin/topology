...
\begin{itemize}
\item\index{discrete space}\emph{Discrete space.}\label{page:Discrete metric}
...
(Unless it is stated otherwise, the real line $\mathbb{R}^2$ will be considered with the Euclidean metric.)
...
if \textit{for any $x_0\in \RR$ and any $\eps > 0$ there exists $\delta > 0$ such that $|f(x_0)-f(x)|\z<\eps$, whenever $|x_0-x|<\delta$}.
...
A subset $V$ in a metric space $\c{X}$ is called \index{open subset}\emph{open} if for any $x\in V$ there is $\eps>0$ such that $\Ball(x,\eps)\subset V$.
\end{thm}
...
Show that the union of an arbitrary family of open sets is open.
\end{thm}
