...
\parbf{\ref{ex:Rge}}; \ref{SHORT.ex:Rge:top}.
Check the conditions in \ref{def:top-space}.
...
Show that $V$ is not open in $(\RR,\s{T})$, so $f\:(\RR,\s{T})\to(\RR,\s{T})$ is not continuous.
...
Consider a map that vanishes on the Cantor set and sends each remaining interval homeomorphically to the whole $\RR$.

\parbf{\ref{ex:cont-product-constr}.}
Apply the observation together with the facts that $(x,y)\mapsto x+y$, $(x,y)\mapsto x\cdot y$, and $(x,y)\mapsto \max{x,y}=\tfrac12\cdot |x+y|+\tfrac12\cdot |x-y|$ are continous functions on $\RR^2$.
...
It has three points $a=[0]$, $b=[\tfrac12]$, and $c=[1]$ and the open sets are
\[\emptyset,\  \{a,b\},\ \{b,c\},\  \{a,b,c\}.\]

\parbf{\ref{ex:g-homeo}.}
Show and use that $y\mapsto g^{-1}\cdot y$ is the inverse of $x\mapsto g\cdot x$.
...
Conclude that $x\mapsto \tfrac{x}{|x|}$ defines a continous bijection $\partial K\to \SS^2$ and apply \ref{cor:cont-bijection=homeomorphism}.
...
\parbf{\ref{ex:path-connected-component-closed-open}}; \ref{SHORT.ex:path-connected-component-closed-open:open}.
...
\parit{\ref{SHORT.ex:path-connected-component-closed-open:closed}.}
...
Finally observe that every path-connected component in $([0,1]\times[0,1],\s{T})$ is a vertical segnent $\{x\}\times [0,1]$.
