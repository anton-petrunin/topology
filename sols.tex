...
Topologically speaking, T-shirt is a disc with three holes, two for the hands and one for the head;
...
\parit{Commnent.} Note that we used that $\Ball(y,R)\z\ne \Ball(x,r)$,
without this condition there no general restrictions on $r$ in terms of $R$.
...
Show that every two nonempty sets in $\RR_\ge$ intersect.
...
Recall that by definition of inverse, we have $f^{-1}(f(x))=x$ and $f(f^{-1}(y))=y$ for any $x\in \c{X}$ and $y\in\c{Y}$.
...
Assume $\Omega$ can be described by the inequality $r<f_n(\theta)$ in the polar $(r,\theta)$-coordinates, were $f_n\:\mathbb{S}^1\to \RR$ is a continuous function.
...
Do not forget about the second part.
...
Observe that $(V\times \c{Y})\cap(\c{X}\times W) = V\times W$.
...
Thus, $\RR/\RR_+$ contains three points, say $p$, $n$, and $z$ that corespond to positive numbers, negative numbers, and zero.
