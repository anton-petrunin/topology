...
Recall that by the definition of inverse, we have $f^{-1}(f(x))=x$ and $f(f^{-1}(y))=y$ for any $x\in \c{X}$ and $y\in\c{Y}$.
...
Assume $\Omega$ can be described by the inequality $r<f_n(\theta)$ in the polar $(r,\theta)$-coordinates, where $f_n\:\mathbb{S}^1\to \RR$ is a continuous function.
...
such that each $\Omega_n$ can be described by the inequality $r<f_n(\theta)$ in the polar $(r,\theta)$-coordinates with continuous $f_n\:\mathbb{S}^1\to \RR$.
...
the compositions $\Phi_n=\phi_n\circ\dots\circ\phi_1\:\Omega_0\to \Omega_n$ stabilizes for each $x\in\Omega_0$;
...
Use that a base is a collection of open sets.
...
Consider cover of $S$ by intervals $(-c,c)$ for all $c>0$.
...
\parbf{\ref{ex:ucompact}.} Apply the definition of compact set.

\parbf{\ref{ex:nested-compact}.} Apply the finite intersection property.

\parbf{\ref{ex:proper-point}.}
Choose a colletion of closed set $\{Q_\alpha\}$ with finite intersection property in $\c{X}$.
...
\parit{Comments.}
This is a well known puzzle; for example, with following interpretation, it apperas in the book of
