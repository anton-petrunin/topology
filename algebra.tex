...
Applying the definition of the product of paths and composition of maps to $f\circ(a_0*a_1)(t)$ and $(f\circ a_0)*(f\circ a_1)(t)$ we get exactly the same expression:
\begin{align*}
\begin{cases}
...
suppose $f(x_0)=y_0$.
For any loop $a$ based at $x_0$ in $\c{X}$, the composition $f\circ a$ is a loop based at $y_0$ in $\c{Y}$.
Moreover, 
...
\section[About f⁎]{About $\bm{f_*}$}

The induced  homomorphism \index{$f_*$}$f_*\:\pi_1(\c{X},x_0)\to \pi_1(\c{Y},y_0)$ has the same direction as the original map $f\:\c{X}\to \c{Y}$, by that reason it is also called \index{pushforward}\emph{pushforward}.
...
Apart from fundamental group, we will not discuss covariant and contravariant constructions further.
But as an example of contravariant construction, consider the so-called \emph{first cohomotopy group} $\pi^1(\c{X},x_0)$ that is made of homotopy classes of pointed continuous maps $(\c{X},x_0)\to (\SS^1,s_0)$; that is continuous maps $\c{X}\to\SS^1$ that send $x_0$ to $s_0$.
...
The same way we show that $\phi_{\bar c}$ is also a right inverse of $\phi_c$.
