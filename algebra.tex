...
Let $f\:\c{X}\to \c{Y}$ be a continuous map, and let $f(x_0)=y_0$.
...
suppose $f(x_0)=y_0$.
Given a loop $a$ with base at $x_0$ in $\c{X}$, the composition $f\circ a$ is a loop with base at $y_0$ in $\c{Y}$.
Moreover, 
$f_*\:[a]\mapsto [f\circ a]$
defines a homomorphism $f_*\:\pi_1(\c{X},x_0)\to \pi_1(\c{Y},y_0)$.
\end{thm}

\parit{Remark.}
The induced homomorphism \index{$f_*$}$f_*\:\pi_1(\c{X},x_0)\to \pi_1(\c{Y},y_0)$ has the same direction as the original map $f\:\c{X}\to \c{Y}$.
Notation $f_*$ can be used for \textit{any} induced map of this type; for example,
...
Such induced maps are called \index{covariant}\emph{covariant}.
If the induced map goes in the opposite direction, then it is denoted by $f^*$; such induced maps are called \index{contravariant}\emph{contravariant}.
...
The same way we show that $\phi_c$ is a left inverse of $\phi_{\bar c}$.
