...
for a fixed $g\in G$ are called \emph{left} and \emph{right cosets} of $H$.
The number of left cosets is called the \emph{index} of $H$ in $G$;
...
\index{kernel}\emph{kernel} and
\index{image}\emph{image} are defined by
\begin{align*}
...
Moreover, they are \emph{cyclic}; that is, any element of the group can be written as a power of a certain fixed element.

For commutative groups, one often uses \emph{additive notation};
...
So we will use multiplicative notation (as in the previous section) for general groups, but once we know that the group is commutative, we will mostly switch to the additive notation --- once we switch, the following changes occur:
