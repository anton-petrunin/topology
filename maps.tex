...
are continuous.%
\footnote{The term \textit{homomorphism} from abstract algebra looks similar and it has similar meaning but should not be confused with \textit{homeomorphism}.}
...
Prove that the complement of a circle in the Euclidean space is homeomorphic to the Euclidean space without a line $\ell$ and a point $p\not\in\ell$.
\end{thm}
...
Note that a homeomorphism can be defined as a \textit{continuous open bijection}.
...
Show that any function $\RR\to\RR$ that is closed and open has to be continuous.
\end{thm}
