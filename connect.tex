...
Suppose $V$ is a \index{proper subset}\emph{proper} clopen subset of a topological space $\c{X}$; that is, $V\ne\emptyset$ and $V\ne\c{X}$.
...
A subset of a topological space is called connected or disconnected if the corresponding subspace is connected or disconnected, respectively.
Spelling out the notion of subspace we get the following definition.
...
A pair of open sets $V$ and $W$ as in the definition will be called an \index{open splitting}\emph{open splitting} of $A$.
...
Therefoe, it converges; denote its limit by $x$.
...
Conclude that any continuos map $\RR\to \ZZ$ is constant;
...
Note that the space $\c{X}$ is connected if and only if $\c{X}$ is a connected component of some (and therefore of any) point in $\c{X}$.
...
Evidently, the number of connected components is a topological invariant;
...
Consider that action on $Q$ of the group $H$ of all homeomorphisms $Q\to Q$.
...
\section{Open-close argument}

The followowing exercise gives an example of application of the so-called \emph{open-close argument}.

\begin{thm}{Exercise}
Let $f\:[0,1]\times[0,1] \to [0,1]\times[0,1]$ is an arbitrary map that increases the firs coordinate;
that is, if $(x_1,y_1)=f(x_0,y_0)$ then $x_1>x_0$.
Show that there is a path $h\:[0,1]\to[0,1]\times[0,1]$ that connects the point $(0,0)$ to some point $(1,y)$ such that for any any point $h(t)$ for $t\in [0,1)$ lies on a line segment from $(x,y)$ to $f(x,y)$
\end{thm}
