...
Consider all continuous maps $\SS^1\to \c{X}$ that send $u_0$ to $x_0$.
Let us denote by $\pi_1(\c{X},x_0)$ the set of homotopy classes of these maps within the described class of maps.
In this chpter we will equip this set with multiplication and show that it is indeed a group, after that we will have right to call it the fundamental group $\c{X}$ with marked point $x_0$.
...
By homotopy of paths we will understand homotopy realive to the ends of $[0,1]$;
in the notations of \ref{sec:rel-homotopy}, we have $\c{X}=[0,1]$ and $A=\{0,1\}$.
If we need to talk about general homotopy of paths we say \index{free homotopy}\emph{free homotopy}.
