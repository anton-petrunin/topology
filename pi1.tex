...
Recall that homotoy was defined in Section \ref{sec:homotopy}; here we introduce its generalized version.

Let $A$ be a subset a topological space $\c{X}$, and let $\c{Y}$ be another topological space.
Two maps $f,g\: \c{X}\to \c{Y}$ are called \index{homotopic relative to $A$}\emph{homotopic relative to $A$}, briefly, $f\sim g\rel A$, if there exists a homotopy
\[
H\:\c{X}\times[0,1]\to \c{Y}
\]
from $f$ to $g$ that is constant on $A$;
that is, $H(a,t)$ is constnat for any $a\in A$ and $t$.
In this case, the map $H\:\c{X}\times[0,1]\to \c{Y}$ is called \emph{homotopy reative to $A$} from $f$ to $g$.

If $H$ is a homotopy reative to $A$ from $f$ to $g$, then each map $f$ and $g$ is a restriction of $H$.
In particular, both maps $f$ and $g$ are continuous.
Note also that $f$ agrees with $g$ on $A$;
that is, $f(a)=g(a)$ for any $a\in A$.

The properties of homotopies that was described in the previous chapter admit straightforward generalization to its relative version.
Mainly, $\sim\rel A$ defines an equivalence relation on the space of maps from $\c{X}$ to $\c{Y}$; in particular, we can talk about its equivalence classes.

\section{Informal description}

Recall that $\SS^1$ denotes the unit circle.
Let $\c{X}$ be a topological space with marked point $x_0\in \c{X}$.
Choose a point $s_0$ on the circle $\SS^1$.
Consider all maps $\SS^1\to \c{X}$ that map $u_0$ to $x_0$;
let us denote by $\pi_1(\c{X},x_0)$ the set of homotopy classes of these mas relative to $\{u_0\}$.
...
Let us apply the definition of realtive homotopy for unit segment with its ends;
