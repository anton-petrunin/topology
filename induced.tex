...
Two maps $f\:\c{X}\to\c{Y}$ and $g\:\c{X}\to\c{Z}$ are coninuous if and only if
the $x\mapsto (f(x),g(x))$ defines a continous map $f\:\c{X}\to\c{Y}\times \c{Z}$.
\end{thm}


\begin{thm}{Exercise}\label{ex:cont-product-constr}
Let $f$ and $g$ be continous real-valued functions on the topological space $\c{X}$.
Sho that (a) $h_1=f+g$, (b) $h_2=f\cdot g$, and (c) $h_3(x)\df\max\{\,f(x), g(x)\,\}$ are continuous
