...
Recall the \emph{restriction} of a map $f:\c{Y}\to \c{Z}$ to a subset $A$ is obtained by keeping the same rule as $f$ but only allowing inputs from~$A$.
It is usually denoted by $f|_A$; so $(f|_A)(a)=f(a)$ if $a\in A$ and otherwise it is undefined.

Observe that if $f\:\c{Y}\to \c{Z}$ is a continuous map, then for any subspace $A\subset \c{Y}$, the restriction of $f$ to $A$ is continuous.
...
Show that this is any not longer true without assumptions of \ref{SHORT.ex:u-continuous:open} or \ref{SHORT.ex:u-continuous:closed}.
\end{subthm}
