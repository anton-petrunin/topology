...
Declare a subset $V\subset\c{X}$ to be open in the \index{induced topology}\emph{induced topology} for $f$ if there is an open subset $W\subset\c{Y}$ 
such that $V=f^{-1}(W)$.
...
The constructed topology on $\c{Y}$ is called \index{final topology}\emph{final} topology.
...
The initial topology is used mostly for injective maps; in this case, it is nearly the same as \index{induced topology}\emph{induced topology}.
Similarly, final topology is mostly used for surjective maps.
This particular case of the construction is called \index{quotient topology}\emph{quotient topology};
...
The corresponding final topology on $\c{X}/{\sim}$ is called \index{quotient topology}\emph{quotient topology} on $\c{X}/{\sim}$.
By default, $\c{X}/{\sim}$ is equipped with the quotient topology
in this case, it is called \index{quotient space}\emph{quotient space}.
...
By that reason $\c{X}/G$ is also called \index{orbit space}\emph{orbit space}.
